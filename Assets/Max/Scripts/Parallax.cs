...
public class Parallax : MonoBehaviour {
	...
	void Start () {
		xGradient = ((bottomRight.position.x - 14f) - (topLeft.position.x + 14f))/
		...
		yGradient = ((bottomRight.position.y + 14.7f) - (topLeft.position.y - 13.3f))/
	...
	void Update () {
		transform.position = new Vector3(xGradient * cameraTrans.position.x + xIntercept,
		                                 yGradient * cameraTrans.position.y + yIntercept, 6f);
	}
}
